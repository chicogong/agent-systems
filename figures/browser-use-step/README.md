...
图从上往下读，四条泳道依次是 Agent、BrowserSession + Tools、模型、AgentHistory。Agent 请求浏览器状态；浏览器返回包含 DOM/URL/可用截图的 `BrowserStateSummary`；Agent 的消息管理器构造输入交给模型；模型返回动作列表；Agent 经 `multi_act`、`Tools.act` 在浏览器侧执行；`ActionResult` 回到 Agent；在有结果的情况下 `_finalize` 追加 `AgentHistory`。箭头是教学整理的主路径，不表示所有步骤都一定发生或异步组件只有一种实际调度顺序。
