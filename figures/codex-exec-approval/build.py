...
def tile(scene: Scene, name: str, x: int, y: int, w: int, h: int,
    ...
    scene.text(f"{name}-title", title, x + 19, y + 15, w - 38, 23, stroke, 8)
    scene.text(f"{name}-detail", detail, x + 19, y + 53, w - 38, 17, INK, 8)
...
def build():
    ...
    d.text("subtitle", "exec_command 的审批决定、首次沙箱尝试和停止路径", 58, 89, 1150, 19, MUTED, 8)
    ...
    d.text("conditional", "仅符合重试条件", 767, 810, 125, 16, "#9a6700", 8)
    d.text("footer", "网络审批、平台后端、apply_patch 拦截另论；虚线仅表示条件路径。", 58, 928, 1160, 16, MUTED, 8)
