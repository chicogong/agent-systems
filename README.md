...
这是一部持续写作、计划开放发布的指南，不是某个 Agent 项目的说明书。**机制是主线，项目是有版本边界的案例，横向对照解释不同实现的取舍。** 当前仓库是私有预览筹备稿；静态源码阅读不等于运行验证，也不代表已完成整本书的外部试读或正式出版。
...
| 方式 | 用途 |
| --- | --- |
| [在线阅读网站](https://books.aimake.cc/) | 已发布本轮书稿的 51 个正文单元及必要索引；网站可公开阅读，但 GitHub 源码仓、可编辑图源与 PDF 下载仍未开放。 |
...
本轮候选包含 12 篇机制专题、14 个固定源码版本的系统局部剖面、4 篇跨系统对照和 24 组可编辑图。正文虽已开放在线阅读，源码仓仍是**私有预览稿**：更多同题架构对照、打印样张、运行实验、二次来源复核及外部读者验收仍待完成。100 页 A4 PDF 已完成本轮电子版校样，但不等于印刷母版。[当前状态与下一步](docs/roadmap.md)是唯一进度入口；[选题地图](docs/program.md)收录候选，但列入候选不等于已经核验。
