...
这是一条**不需要 API Key、仅用 Python 标准库**的练习。目标不是做一个有用的编码助手，而是亲手观察一个容易被“Agent 已完成”掩盖的区别：**提案由模型侧给出；能否执行由宿主决定；工具只报告这次调用；候选成果还需按原任务验证。**代码里的 `propose()` 是固定脚本，不是 LLM，因此实验只验证控制流，不证明真实模型的推理质量。

任务是：把配置文件中的 `timeout` 从 30 改为 5，保持 `retries=3`。要求先读配置、获准后再写、检查两个条件，最后只报告“可提交验收”或“受阻”；**没有用户接受环节**。程序在系统临时目录创建 `config.json`，运行结束即清理，不改真实项目文件，也不调用网络。
...
在正常模式中，第二次提案的 `seen_results` 是 1，说明提案函数拿到了 `read` 的结果；第三次提案收到写入结果后才要求 `check`。这模拟**下一步基于观察产生**，却不代表脚本会像真实模型一样自由规划。写入是练习宿主的本地工具，审批发生在调用前；`finish` 只是提案函数希望停止，外层 `run()` 仍独立检查配置，再将它标为 `candidate_ready`。这个状态不是“用户已接受”。
...
3. `execute()` 是唯一读写临时配置文件的工具入口。它的 `ok=true` 仅表示这次工具调用完成；`write` 并不知道用户是否要求保留重试规则。
