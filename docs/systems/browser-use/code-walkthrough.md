...
5. `_finalize()` 在有 `last_result` 时调用 `_make_history_item()`；后者从当前浏览器摘要保存 URL、标题、标签页、交互元素和可用截图路径，并与模型输出、动作结果、step 元数据组成 `AgentHistory`。模型输出存在时，`_finalize()` 还派发 `CreateAgentStepEvent`。[最终处理](https://github.com/browser-use/browser-use/blob/d8110c5ff87ccba887aaa726cdb780f2f84bef8d/browser_use/agent/service.py#L1356-L1415) · [历史项构造](https://github.com/browser-use/browser-use/blob/d8110c5ff87ccba887aaa726cdb780f2f84bef8d/browser_use/agent/service.py#L1738-L1780)
