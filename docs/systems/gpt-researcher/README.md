...
2. Hybrid 分支先用 `OnlineDocumentLoader` 或 `DocumentLoader` 读取本地/指定文档，再以 `asyncio.gather` 并发执行“给定文档的上下文提取”和“空文档输入的网页检索”，最后用 `prompt_family.join_local_web_documents` 拼接。第一路**只有文档非空时**才是本地文档上下文；若加载结果为空，`_process_sub_query()` 可能转向网页抓取，因此两路都可能取得网页内容。[来源分支](https://github.com/assafelovic/gpt-researcher/blob/6f998577d547b1e54ec662dac63583aa11e3b84b/gpt_researcher/skills/researcher.py#L164-L196) · [空文档回退](https://github.com/assafelovic/gpt-researcher/blob/6f998577d547b1e54ec662dac63583aa11e3b84b/gpt_researcher/skills/researcher.py#L510-L625) · [拼接格式](https://github.com/assafelovic/gpt-researcher/blob/6f998577d547b1e54ec662dac63583aa11e3b84b/gpt_researcher/prompts.py#L564-L566)
