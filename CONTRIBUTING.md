...
欢迎帮助改正源码定位、图解、文字、Markdown 阅读包和打印问题。请先读[写作与校稿方法](docs/editorial-plan.md)及[来源规则](sources/README.md)；当前进度只在[路线页](docs/roadmap.md)维护。本仓库欢迎具体反例和可复核的修正，不接受无来源的项目排名或把静态源码解读写成运行事实。
...
最后两步会重建生成图源；请先检查自己的工作区改动，不要在有未保存的手工图稿编辑时执行。书稿改动还应按[构建说明](book/README.md)检查 Markdown 阅读包与 PDF 的受影响内容。CI 是结构检查，不是源码审稿、视觉验收或许可批准。
